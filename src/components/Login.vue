    ...
    <div class="contain py-16">
        <div class="max-w-lg mx-auto shadow px-6 py-7 rounded overflow-hidden">
            ...
            <p class="text-gray-600 mb-6 text-sm">
                <span v-if="authStore.errorMessage.message" class="flex items-center font-medium tracking-wide text-red-500 text-xs mt-1 ml-1">{{ authStore.errorMessage.message }}</span>
            </p>
            <form action="#" method="post" autocomplete="off">
                ...
                <div class="mt-4">
                    <button type="button" @click="auth.authenticate(email, password)"
            ...
            <p class="mt-4 text-center text-gray-600">Don't have account? <router-link to="register" class="text-primary">Register now</router-link></p>
        </div>
    </div>
    <!-- ./login -->
</template>